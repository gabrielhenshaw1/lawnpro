       ...
       {selectedImage && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-90" onClick={() => setSelectedImage(null)}>
          <div className="relative max-w-4xl max-h-screen p-4">
            ...
            <button className="absolute top-4 right-4 bg-white text-black rounded-full w-10 h-10 flex items-center justify-center font-bold hover:bg-gray-200" onClick={() => setSelectedImage(null)}>✕</button>
          </div>
        </div>
      )}
                   ...
                   <div className="bg-gray-50 p-6 w-full md:w-80 border-l border-gray-200 flex flex-col justify-center gap-3">
                      ...
                      <div className="flex gap-2">
                         ...
                         <div className="flex-1">
                            ...
                            <input type="number" className="w-full p-2 border rounded" placeholder="1.0" step="0.5"
                              onChange={e => setDurationInputs(prev => ({...prev, [req.id]: e.target.value}))} />
                         </div>
                      </div>
                      <button onClick={() => handleAction(req.id, 'quote', { amount: quoteInputs[req.id], duration: durationInputs[req.id] })} 
                        className="w-full bg-blue-600 text-white py-2 rounded font-bold hover:bg-blue-700">
                        Send Quote
                      </button>
                      <div className="flex gap-2 mt-2">
                         <button onClick={() => handleAction(req.id, 'approve')} className="flex-1 text-green-600 hover:underline text-sm font-medium">Quick Approve</button>
                         <button onClick={() => handleAction(req.id, 'deny')} className="flex-1 text-red-500 hover:underline text-sm font-medium">Deny</button>
                      </div>
                   </div>
                 )}
